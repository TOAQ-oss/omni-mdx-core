...
⚠️ **Warning for Client-Side Plugins:** Ensure the plugins you provide to the client parser are browser-safe. Some `rehype` plugins rely on Node.js native APIs (`fs`, `path`) and will crash your bundler (Webpack/Turbopack) if imported into a `"use client"` file.
