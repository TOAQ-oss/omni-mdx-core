...
fn check_no_hidden_bytes(nodes: &[AstNode]) {
    for node in nodes {
        if let Some(ref content) = node.content {
            assert!(!content.contains('\x01'), "Byte \\x01 trouvé dans le nœud {}: {:?}", node.node_type, content);
            assert!(!content.contains('\x04'), "Byte \\x04 trouvé dans le nœud {}: {:?}", node.node_type, content);
            assert!(!content.contains('\x05'), "Byte \\x05 trouvé dans le nœud {}: {:?}", node.node_type, content);
        }
...
```"#;
    ...
    assert!(!has_node_type(&ast, "Component"), "<Component> was incorrectly extracted from fenced block");
}
...
```
    ...
    assert!(!has_node_type(&ast, "div"), "div incorrectly extracted from HTML code block");
}
...
fn test_inline_code_is_protected() {
    ...
    assert!(!has_node_type(&ast, "Note"), "Note extracted from inline code");

    let p = find(&ast, "p").expect("paragraph not found");
    assert!(p.children.iter().any(|c| c.node_type == "code"), "Inline code node missing");
}
...
"#;
    ...
    let note_type = note.attributes.as_ref()
        ...
        .and_then(|v| if let AttrValue::Text(s) = v { Some(s.as_ref()) } else { None });
...
"#;
    ...
    assert!(!has_complex_math, "Math inside code block was incorrectly extracted");
    assert!(inline_maths.iter().any(|m| m.content.as_deref() == Some("x^2")), "Real math after code block missing");
}
